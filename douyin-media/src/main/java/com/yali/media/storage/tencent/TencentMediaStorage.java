package com.yali.media.storage.tencent;
...
import com.yali.media.config.TencentProperties;
...
import com.yali.media.storage.MediaUploadResult;
...
import static com.yali.media.enums.FileErrorInfo.Msg.*;
...
public class TencentMediaStorage implements IMediaStorage {
    ...
    @Override
    public List<RemoteMedia> queryMediaInfos(String... fileIds) {
        // 1.请求参数
        ...
        // 4.数据转换
        List<RemoteMedia> list = new ArrayList<>(mediaInfoSet.length);
        for (MediaInfo info : mediaInfoSet) {
            RemoteMedia remoteMedia = new RemoteMedia();
            remoteMedia.setFileId(info.getFileId());
            MediaMetaData md = info.getMetaData();
            MediaBasicInfo bi = info.getBasicInfo();
            remoteMedia.setMediaUrl(bi.getMediaUrl());
            remoteMedia.setFilename(bi.getName());
            remoteMedia.setSize(md.getSize());
            remoteMedia.setDuration(md.getDuration());
            remoteMedia.setStatus(FileStatus.UPLOADED);
            list.add(remoteMedia);
        }
        return list;
    }
}
